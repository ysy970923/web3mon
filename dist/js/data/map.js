...
import { battleMapBoundaries, canvas, mainMapBoundaries, player, setBoundaries } from '../../js/index';
import { mainBackgroundImage, foregroundImage, battleBackgroundImage } from '../../js/load';
import { ws } from '../../js/network';
import { boundaries, battleZones, characters, setMovables, setRenderables } from '../../js/index';
...
export var background = new Sprite({
  position: {
    x: offset.x,
    y: offset.y
  },
...
export var foreground = new Sprite({
  position: {
    x: offset.x,
    y: offset.y
  },
...
export function transferMapTo(toMap) {
  ...
  if (toMap === MAP.TEST) {
    ...
    setMovables([background].concat(_toConsumableArray(boundaries), _toConsumableArray(battleZones), _toConsumableArray(characters), [foreground]));
    ...
    setRenderables([background].concat(_toConsumableArray(boundaries), _toConsumableArray(battleZones), _toConsumableArray(characters), [player, foreground]));
    setMovables([].concat(_toConsumableArray(boundaries), [background], _toConsumableArray(battleZones), _toConsumableArray(characters), [foreground]));
    moveToPosition(200, -100);
    var _msg = JSON.stringify(_body);
    ws.send(_msg);
  }
}
var showMapLoading = /*#__PURE__*/function () {
  ...
  return function showMapLoading() {
