  ...
  switch (type) {
    ...
    case 'your_player_id':
      ...
      if (!isMyEntrance) {
        ...
        // ])
        makeOthers(data, [window.innerWidth / 2 - 192 / 4 / 2, window.innerHeight / 2 - 102 / 2]);
      } else {
        myID = data;
    ...
    case NETWORK.MAP_STATUS:
      ...
      data['player_infos_for_view'].forEach(function (avatar) {
        ...
        if (!(avatar.player_id in others || avatar.player_id === myID)) {
          ...
          makeOthers(avatar.player_id, avatar.coordinate, String(avatar.player_id).slice(0, 5), avatar.nft_image_url);
        }
      });
      break;
    case ACTION.MOVE:
      if (data.player_key === myID) {
        ...
        var _id = data.player_key;
        if (data.coordinate[0] === 1 && data.coordinate[1] === 1) {
          others[_id].sprite.animate = false;
        } else {
          // 디렉션 계산해서 이미지 부여하기
          var newPosition = local_position({
            x: data.coordinate[0],
            y: data.coordinate[1]
          });
          var isLeft = others[_id].sprite.position.x - newPosition.x < -1;
          var isBottom = others[_id].sprite.position.y - newPosition.y < -1;
          var isRight = others[_id].sprite.position.x - newPosition.x > 1;
          var isUp = others[_id].sprite.position.y - newPosition.y > 1;
          ...
          moveObject(_id, newPosition, isLeft | isRight, isUp | isBottom);
...
export function reportError(errMessage) {
  log_error("Error ".concat(errMessage.name, ": ").concat(errMessage.message));
}
...
export function connect() {
  var serverUrl;
  var scheme = 'ws';
  var hostName = 'ec2-44-201-5-87.compute-1.amazonaws.com:8080/ws';
